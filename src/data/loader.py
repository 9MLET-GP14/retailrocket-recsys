...
class RetailRocketLoader:
    ...
    def __init__(self, raw_path: str | None = None) -> None:
        ...
        logger.info("RetailRocketLoader initialized with path: %s", self._raw_path)
    ...
    def load_events(self) -> pd.DataFrame:
        ...
        df = pd.read_csv(path)
        logger.info("Raw events loaded: %d rows", len(df))
    ...
    def load_item_properties(self) -> pd.DataFrame:
        ...
        logger.info("Loading item properties from %d files", len(parts))
        df = pd.concat([pd.read_csv(p) for p in parts], ignore_index=True)
        logger.info("Item properties loaded: %d rows, %d unique items",
    ...
    def _validate_events(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            "Events validated: %d → %d rows | distribution: %s",
            n_before,
            len(df),
